...
public class HistoryCallTableSelectionListener implements ListSelectionListener, TableModelListener {

	private TableModel model;
	private ListSelectionModel selection;

	public HistoryCallTableSelectionListener(TableModel model,
	...
	public void valueChanged(ListSelectionEvent e) {
		if (!e.getValueIsAdjusting()) {
			if (model.getRowCount() > 0 && selection.isSelectionEmpty()) {
				...
				if (row > -1) {
					HistoryCall historyCall = ((HistoryCallTableModel)model).getRow(row);

					selection.setValueIsAdjusting(true);
					selection.setValueIsAdjusting(false);

					MessageService
							.sendMessage(new SelectionMessage(this, historyCall));
				}else{
					MessageService
					.sendMessage(new SelectionMessage(this, null));
				}
	...
	public void tableChanged(TableModelEvent e) {
		...
		if (e.getType() == TableModelEvent.UPDATE) {
			...
			if (row > -1 ) {
				...
				MessageService
					.sendMessage(new UpdateMessage(this, historyCall));
			}
