 ...
 * historico de llamada sip. El dialogo contiene  todos los componentes
 * de un historico de llamada sip.
 * @author jmdomingueza
 *
 */
public class ShowHistoryCallSipDialog extends ShowHistoryCallDialog {
	
	private static final long serialVersionUID = -4510834058365692418L;
	
	private static final Log logger = LogFactory.getLog(ShowHistoryCallSipDialog.class);
	 ...
	 * @param call
	 */
	public ShowHistoryCallSipDialog(Window owner, String title, int width, int height,HistoryCall historyCall) {
		super(owner, title, width, height,historyCall);
		
		logger.trace("Enter ShowHistoryCallSipDialog");
		
		this.init();
		
		logger.trace("Exit ShowHistoryCallSipDialog");
	}
	...
	public static int showDialog(Window owner, String title,HistoryCall historyCall){
		...
		dialog = new  ShowHistoryCallSipDialog(owner,title,d.width/4,d.height/4,historyCall);
		
		DialogIUtils.centerDialog(owner, dialog);
		DialogIUtils.showDialog(dialog);
		
		logger.trace("Exit showDialog");
		return ((ShowHistoryCallSipDialog)dialog).getSelectedOption();
	...
	protected  JPanel createGeneralPanel() {
		...
		idLabel = ComponentFactory.createLabel("main.dialog.showhistorycallsip.label.id");
		idTextField = ComponentFactory.createTextField(historyCall.getId()+"",20,false);
		
		countLabel = ComponentFactory.createLabel("main.dialog.showhistorycallsip.label.count");
		countTextField = ComponentFactory.createTextField(historyCall.getCount(),20,false);
		
		numberLabel = ComponentFactory.createLabel("main.dialog.showhistorycallsip.label.number");
		numberTextField = ComponentFactory.createTextField(historyCall.getCount(),20,false);
		
		stateLabel = ComponentFactory.createLabel("main.dialog.showhistorycallsip.label.state");
		stateTextField = ComponentFactory.createTextField(historyCall.getState().toString(),20,false);
		
		directionLabel = ComponentFactory.createLabel("main.dialog.showhistorycallsip.label.direction");
		directionTextField = ComponentFactory.createTextField(historyCall.getDirection().toString(),20,false);
		
		descriptionLabel = ComponentFactory.createLabel("main.dialog.showhistorycallsip.label.description");
		descriptionTextField = ComponentFactory.createTextField(historyCall.getDescription(),20,false);
		
		dateLabel = ComponentFactory.createLabel("main.dialog.showhistorycallsip.label.date");
		SimpleDateFormat dateFormat = new SimpleDateFormat(resourceService.getProperty("main.dialog.showhistorycallsip.date.format"));
		String value;
