...
public class CallTableSelectionListener implements ListSelectionListener, TableModelListener {

	private TableModel model;
	private ListSelectionModel selection;

	public CallTableSelectionListener(TableModel model,
	...
	public void valueChanged(ListSelectionEvent e) {
		if (!e.getValueIsAdjusting()) {
			if (model.getRowCount() > 0 && selection.isSelectionEmpty()) {
				...
				if (row > -1) {
					Call call = ((CallTableModel)model).getRow(row);

					selection.setValueIsAdjusting(true);
					selection.setValueIsAdjusting(false);

					MessageService
							.sendMessage(new SelectionMessage(this, call));
				}else{
					MessageService
					.sendMessage(new SelectionMessage(this, null));
				}
